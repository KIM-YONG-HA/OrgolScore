...
public class MainContoller2 {
	...
	public MainContoller2() {
		...
		customFont = setCustomFont("PretendardVariable", 16);
...
//		}
	...
	// 플레이바 생성 
	public void createPlayBar(JPanel wrap) {

		JPanel playBarArea = new JPanel();
		//playBarArea.setPreferredSize(new Dimension(600, 50));
		playBarArea.setLayout(new GridLayout());
		playBarArea.setBounds(0, 0, 600, 50);
		...
		for (int i = 0; i < btns.length; i++) {
			...
			buttonArray[i].setFont(setCustomFont("PretendardVariable", 16));
	...
	public void currentMeasureDisplay() {
	    new Thread(() -> {
	        try {
	            int totalColumns = 256; // 총 열의 개수
	            int totalRows = 29; // 총 행의 개수
	            int columnDuration = 500; // 각 열 재생 시간 (밀리초)

	            for (int column = 0; column < totalColumns; column++) {
	                for (int row = 0; row < totalRows; row++) {
	                    int labelIndex = row * totalColumns + column;

	                    // 이전 열 복구
	                    if (column > 0) {
	                        int prevLabelIndex = row * totalColumns + (column - 1);
	                    ...
	                    currentLabel.setForeground(new Color(0xffffff)); // 글자색
	                }

	                // 열 재생 시간 대기
	                Thread.sleep(columnDuration);
	            }

	            // 마지막 열 복구
	            for (int row = 0; row < totalRows; row++) {
	                int lastLabelIndex = row * totalColumns + (totalColumns - 1);
	                JLabel lastLabel = midiLabels.get(lastLabelIndex);
	                String pos = (String) lastLabel.getClientProperty("pos");

	                if ("odd".equals(pos)) {
	                    lastLabel.setBackground(new Color(0xECF0F5)); // odd color
	                    lastLabel.setForeground(new Color(0xECF0F5));
	                } else {
	                    lastLabel.setBackground(new Color(0xB6C3CF)); // even color
	                    lastLabel.setForeground(new Color(0xB6C3CF));
	                }
	            }
...
//        });
	...
	public static void playVstSound(String scale) {
		...
		try {
			...
			// 클래스 로더를 통해 리소스를 가져옴
			InputStream inputStream = MainContoller2.class.getResourceAsStream(filePath);
